(function (window, undefined) {
    var count = 0, inputCount = 0, ynTrees = {}, classNameCfg = {spread: "spread", shrink: "shrink"},
        inputFlag = Math.ceil(Math.random() * 100000000);

    function YnTree(ele, options, configs) {
    ...
    YnTree.prototype._createDom = function (data, parent) {
        ...
        YnTree.forEach(data, function (index, item) {
            var html = "", id = "yn_tree_input" + ++inputCount + '_' + inputFlag, nameVal = item.name ? item.name : "",
    ...
    CompositeLeaf.prototype = {
        constructor: CompositeLeaf, selectDown: function (flag) {
            ...
            if (ynTree.configs.spreadChecked) {
                // 展开选中选项
                if (this.dom.childNodes[2]) {
                    this.dom.classList.add('spread');
                    this.dom.classList.remove('shrink');
                }
                var parentLi = this.dom.parentElement.parentElement;
                while (parentLi.className.indexOf('yn-tree-li') >= 0) {
                    parentLi.classList.add('spread');
                    parentLi.classList.remove('shrink');
